...
function init() {
  ...
  queue.loadManifest([
    {id:"spaceOne", src:"images/universe-background-alt.jpg"}
  ]);
  ...
  WebFont.load({
    ...
    fontactive: () => {
      ...
      subtitleField = new createjs.Text(`${leftArrow} ${rightArrow} to Aim`, "lighter 32px 'Space Mono'", "#FFC41E");
    	...
    	subtitleField.y = canvas.height / 2 + 100;
      subtitleFieldTwo = new createjs.Text(`SPACE to Fire`, "lighter 32px 'Space Mono'", "#FFC41E");
    	...
    	subtitleFieldTwo.y = canvas.height / 2 + 160;
...
function nextLevel() {
  ...
  if (points > nextBonus) {
    ...
    nextBonus = nextBonus * 2 + nextBonus;
