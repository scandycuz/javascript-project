...
function nextLevel() {
  ...
  setTimeout( () => {
    ...
    document.onkeydown = handleKeyDown;
    canvas.onclick = restart(() => levelComplete = false);
    document.addEventListener("keydown", restart(() => levelComplete = false));
  }, 800);

}

function restart(resetLevelComplete) {
  ...
  if (resetLevelComplete) { resetLevelComplete() };
}
...
function tick(event) {
  ...
  if (won()) {
    console.log('changed');
    console.log(`won, level complete is ${levelComplete}, and won is ${won()}`);
    ...
    if (gameOver()) {
      ...
      levelComplete = true;
      console.log(`now level complete is ${levelComplete}, and won is ${won()}`);
      return nextLevel();
    }
  } else if (!won() && gameOver()) {
    console.log('didnt win game over');
    gameEnded = true;
    createjs.Ticker.removeEventListener("tick", tick);
    return endGame();
  } else {
...
function fireCannon() {
  if (!levelComplete && !gameLoading && shotCooldown <= 0) {
    shotsThisRound++;
    shotsRemaining--;
    shotCooldown = 10;
    playFire();
